...
function LogicSim() {
    ...
    this.setCanvas = function (canvas) {
        ...
        EventHandler.add(canvas, "mousedown", function (ev) {
            ev = EventHandler.fixMouse(ev);
            this.mouseDown(ev.offsetX, ev.offsetY, ev);
            clearDocumentSelection();
        }.bind(this));
        EventHandler.add(canvas, "mouseup", function (ev) {
            ev = EventHandler.fixMouse(ev);
            this.mouseUp(ev.offsetX, ev.offsetY, ev);
            clearDocumentSelection();
        }.bind(this));
        EventHandler.add(canvas, "mousemove", function (ev) {
            ev = EventHandler.fixMouse(ev);
            this.mouseMove(ev.offsetX, ev.offsetY, ev);
        }.bind(this));
        EventHandler.add(canvas, "click", function (ev) {
            ev = EventHandler.fixMouse(ev);
            this.click(ev.offsetX, ev.offsetY, ev);
            clearDocumentSelection();
        }.bind(this));
        EventHandler.add(window, "keydown", function (ev) {
            this.keyDown(ev);
        }.bind(this));
        EventHandler.add(window, "keyup", function (ev) {
            this.keyUp(ev);
        }.bind(this));
        EventHandler.add(window, "resize", function (ev) {
            this.onResizeCanvas();
        }.bind(this));
        EventHandler.add(canvas, "contextmenu", function (ev) {
            EventHandler.preventDefault(ev);
        });
    };
        ...
        if (mode != this.mode) {
            ...
            switch (mode) {
                case ControlMode.selecting:
                    this.canvas.addClass('selectingCursor');
                    break;
                case ControlMode.deleting:
                    this.canvas.addClass('deletingCursor');
                    break;
                case ControlMode.labeling:
                    this.canvas.addClass('labelingCursor');
                    break;
            }
        }
